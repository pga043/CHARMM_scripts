...
mkdir dockresult dock_pose pdbs
...
total=`sort -gk 2 sorted_total.dat | head -n 1 | awk '{print $1}'`
endall=`date +%s`
#usedall=`expr $endall - $startall`
echo "Time used for one docking trial : $usedall seconds" >> time.log
mv conformer_cluster.log sorted_total.dat time.log result
if [ -s conformer.dat ]; then
	mv conformer.dat result
	fi

## Clean folder
mv result dockresult/run${repeat} 
mv *.pdb pdbs/
